...
import {
  branchLoanDecisions,
  branchHQFingerprints,
  branchFraudAlertHistory,
...
export default function BranchRecordsPage() {
  ...
  const loanFilters = useMemo(() => ({ grade, date_from: dateFrom, date_to: dateTo, limit: 100 }), [
    grade,
    dateFrom,
    dateTo,
  ]);
  const hqFilters = useMemo(() => ({ grade, date_from: dateFrom, date_to: dateTo, limit: 100 }), [
    grade,
    dateFrom,
    dateTo,
  ]);
  const fraudFilters = useMemo(() => ({ severity, date_from: dateFrom, date_to: dateTo, limit: 100 }), [
    severity,
    dateFrom,
    dateTo,
  ]);
  ...
  const refresh = async () => {
    ...
    try {
      const [loan, hq, fraud] = await Promise.all([
        branchLoanDecisions(loanFilters),
        branchHQFingerprints(hqFilters),
        branchFraudAlertHistory(fraudFilters),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-ink">Decision Records</h1>
          <p className="text-sm text-ink-soft mt-1">Persisted loan decisions, HQ fingerprints, and fraud alerts.</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead className="text-ink-soft text-xs uppercase">
                  <tr>
                    ...
                    <th className="text-left py-2">Max Loan</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {loanRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td className="py-2 text-ink">{row.suggested_interest_rate ? `${row.suggested_interest_rate}%` : "-"}</td>
                      <td className="py-2 text-ink">{row.recommended_max_loan_npr ? `NPR ${Math.round(row.recommended_max_loan_npr).toLocaleString()}` : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y">
                  {hqRows.map((row) => (
                    <tr key={row.fingerprint_id}>
                      ...
                      <td className="py-2 text-ink">{row.default_probability ? `${Math.round(row.default_probability * 100)}%` : "-"}</td>
                      <td className="py-2 text-ink">{row.branch_recommended_rate ? `${row.branch_recommended_rate}%` : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y">
                  {fraudRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td className="py-2 text-ink">{typeof row.value === "number" ? row.value.toLocaleString() : "-"}</td>
                      <td className="py-2 text-ink">{row.reason || "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
